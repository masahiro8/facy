...
function drawEyelush(points, rect, textureImg) {
  ...
  console.log("eye points", _points);
  ...
  const getEyelidCurve = points => {
    ...
    getRadian(points[0]);
    getRadian(points[6]);
    console.log("rad1", getRadian(points[0]));
    console.log("rad2", getRadian(points[6]));

    //2点の角度と円のaveCircleの半径を元に円弧を描画してみる

    return aveCircle;
  };
  getEyelidCurve(_points);
  console.log(getEyelidCurve(_points));
